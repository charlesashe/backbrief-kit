...
- Commands: critique, council, find-gap, grade-idea, handoff, next, pickup, setup, verify-install. Compare against the payload's `commands/` folder rather than this sentence where the two disagree. A leftover `resume.md` from before 2.3.0 can never run and should be offered for deletion.
