# Reference library (your knowledgebase)
...
- Only add material you have the right to use (your own files, your purchases for your own use, public documents, freely licensed work).
- Name files so a human can guess the contents: `brand-voice-guide.md` beats `doc-final-v2.pdf`.
- Big libraries are fine, but prune what goes stale. A wrong reference is worse than none.
- Nothing in here is read automatically on every turn; agents check the index and pull from it when a task calls for sources. Point at a specific source in your request when you want it used for sure.

The fastest way to fill this in is to just tell the team: name your documents and links in chat (or during `/setup`) and they get filed and indexed for you.
