...
1. Check each item and print a one-line result per row, pass or fail, no jargon:
   ...
   - `.claude/commands/`: expect next, critique, council, find-gap, grade-idea, handoff, pickup, setup, verify-install, plus the four Business OS signposts intake, business-plan, grade, approve (each answers in two sentences saying the real command ships in the paid bundle; their presence here is correct and is not a Business OS install).
...
3. If Backbrief Business OS is installed instead of this kit, you will also see the cmo, cfo, web, and ops agents, the real (non-signpost) versions of /intake, /business-plan, /grade, and /approve, the ceo-gate and grading rules, and a much larger skill library rather than the base set. That is expected: Backbrief Business OS is the complete bundle and includes everything here.
